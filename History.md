...
Reviewing the old code I realized I had discovered more about the file type than I realized at the time. I was able to make the parser 99% reliable with what I learned from my own code. The JK (Jedi Knight) community pointed out the game engines for JK is open source, and I was able to find the code that reads and executes the Icarus scripts. That gave me the final explanation for a few things I wasn't sure about. DEvaheb 2.0 (the C# remake) was first released in 2022.

## suraci 1.0
In 2026 I decided to pick up the compiler and script validation work. The test framework is now able to decompile the original scripts and then recompile them, and byte-for-byte compare the original IBI with the new one. These tests are now running for all original Star Trek Voyager, Soldier of Fortune 2, Jedi Knight and Jedi Academy files. *(Since the IBI files are part of the games and copyrighted, I cannot include them in this source if you are so inclined to try and run the tests yourself... However you can go to the Helper.cs class in the Tests project and at the top edit the paths to any game files you may have)* This round-tripping both into AST and back, as well as source and back, guarantees that both the IBI parser and Icarus parser are entirely compatible. Sticking with the original idea of reversing the name BehaveED into DEvaheb, I decided to make more poor choices and reverse the name 'icarus' for the compiler executable.